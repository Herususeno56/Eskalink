describe("Dashboard Page Test Cases", () => {
  ...
  it("User memproses pesanan lebih kecil dari gudang", () => {
    cy.get("form input").check(["Tandai"]);

    const button = cy.get("button");
    button.click();
    cy.on("window:alert", (text) => {
      expect(text).to.contains("TerKirim");
    });
  });

  it("User memproses pesanan lebih besar dari gudang", () => {
    cy.get("form input").check(["Tandai"]);
...
it("User membuat dokumen pesanan baru", () => {
